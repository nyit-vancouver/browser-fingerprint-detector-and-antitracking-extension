...
devices.mobile.ios2 = (
  devices.mobile.ios1.map((d: Device) => {
...
devices.mobile.ios3 = (
  devices.mobile.ios2.map((d: Device) => {
...
devices.tablet.ios2 = (
  devices.tablet.ios1.map((d: Device) => {
    return {
      build: '20H115',
      name: d.name,
      viewport: d.viewport,
...
const getDevice = (hardware: string, id: string): Device => {
  return devices[hardware][id][
    Math.floor(Math.random() * devices[hardware][id].length)
  ]
}
