...
const languageMap: Record<string, Omit<Language, 'code'>> = languages.reduce(
  (m: Record<string, Omit<Language, 'code'>>, l: Language) => {
    // eslint-disable-next-line no-param-reassign
    m[l.code] = { name: l.name, nav: l.nav, value: l.value }
    return m
  },
  {}
)
...
let getLanguage = (langCode: string): Language => {
  return { ...languageMap[langCode], code: langCode }
}
