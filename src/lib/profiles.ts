...
// 首先定义一个接口来描述生成的 profile 对象
export interface GeneratedProfile {
  id: string
  // badge: string; // 如果你将来要取消注释这行，请取消这里的注释
  name: string
}
...
const DesktopResolutions: string[] = [
  '1366x768',
  '1440x900',
  '1600x900',
  '1920x1080',
...
const getName = (os: string, browser: string): string => {
  let osId: string = ''
...
export class Generator {
  private browsers = {
    ...
    edg: (os: any): BrowserProfile => {
      ...
      const resolutions: string[] = os.id.includes('mac')
        ? MacResolutions
        : DesktopResolutions
      const screenRes: number[] = resolutions[
        Math.floor(Math.random() * resolutions.length)
      ]
        .split('x')
        .map(Number)

      const ua: string = `Mozilla/5.0 (${platform}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${chromeVersion} Safari/537.36 Edg/${version}`

      const hardwareConcurrency: number = randomHW.includes(os.id)
        ? Math.random() > 0.5
          ? 4
          : 2
        : 4
    ...
    // edge (mobile)
    edgm: (os: any): BrowserProfile => {
      const versions: any = BrowserVersions.edg

      const device = devices.getDevice('mobile', os.id)

      const screenRes: number[] = device.viewport.split('x').map(Number)
      const ua: string = `Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${versions.androidChrome} Mobile Safari/537.36 EdgA/${versions.android}`
    ...
    // firefox esr
    esr: (os: any): BrowserProfile => {
      const version: string = BrowserVersions.esr.desktop
      ...
      const ua = `Mozilla/5.0 (${platform}; rv:${version}.0) Gecko/20100101 Firefox/${version}.0`
    ...
    esr2: (os: any): BrowserProfile => {
      let version: string = BrowserVersions.esr2.desktop
      let appVersion: string = ''
      let platform: string = ''

      let resolutions: string[] = os.id.includes('mac')
        ? MacResolutions
        : DesktopResolutions
      let screenRes: number[] = resolutions[
        Math.floor(Math.random() * resolutions.length)
      ]
        .split('x')
        .map(Number)

      let hardwareConcurrency: number = randomHW.includes(os.id)
        ? Math.random() > 0.5
          ? 4
          : 2
        : 4
    ...
    // internet explorer
    ie: (os: any): BrowserProfile => {
      let resolutions = DesktopResolutions.slice(0, 4)
      let screenRes = resolutions[
        Math.floor(Math.random() * resolutions.length)
      ]
        .split('x')
        .map(Number)

      let availHeight: number = 0
    ...
    sf: (os: any): BrowserProfile => {
      ...
      let screenRes = MacResolutions[
        Math.floor(Math.random() * MacResolutions.length)
      ]
        .split('x')
        .map(Number)
  ...
  getRandomByDevice(device: string): string {
    ...
    if (device === 'random') {
      profilesCanUse = this.profileIds.desktop
        .concat(this.profileIds.mobile)
        .filter((p) => !this.excludedProfiles.includes(p))
    } else if (device === 'randomDesktop') {
      profilesCanUse = this.profileIds.desktop.filter(
        (p) => !this.excludedProfiles.includes(p)
      )
    } else {
